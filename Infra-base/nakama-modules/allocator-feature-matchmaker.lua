...
local function allocate_and_notify(context, matched_users)
    ...
    local body = nk.json_encode({
        namespace = "default", -- Change this to "feature-matchmaker" if your GS are in different K8s namespaces
        scheduling = "Packed",
        selectors = {
            {
                matchLabels = {
                    ["agones.dev/fleet"] = "fleet-feature-matchmaker"
                }
            }
        },
        metadata = { 
            labels = { ["agones.dev/fleet"] = "fleet-feature-matchmaker" } 
        }
    })
    ...
    if http_code == 429 then
        nk.logger_error("AGONES CAPACITY: No Ready GameServers available for feature-matchmaker")
    else
